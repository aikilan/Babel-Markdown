...
class OpenAITranslationClient(
  private val debugListener: TranslationDebugListener? = null,
) {
  ...
  suspend fun translate(request: TranslateRequest): RawTranslationResult {
    ...
    val response = try {
      ...
      emitDebugEvent(
        ...
        responseBody = null,
        status = null,
        errorMessage = "Translation request failed due to a network error.",
        latencyMs = System.currentTimeMillis() - start,
      )
      throw TranslationProviderException(
      ...
      emitDebugEvent(
        ...
        responseBody = null,
        status = null,
        errorMessage = "Translation request failed due to an unknown error.",
        latencyMs = System.currentTimeMillis() - start,
      )
      throw TranslationProviderException(
    ...
    response.use { httpResponse ->
      ...
      if (!httpResponse.isSuccessful) {
        val errorBody = responseBody.trim()
        ...
        emitDebugEvent(
          ...
          status = status,
          errorMessage = "Translation API responded with $status.",
          latencyMs = System.currentTimeMillis() - start,
        )
        throw TranslationProviderException(
      ...
      if (content.isEmpty()) {
        emitDebugEvent(
          ...
          responseBody = responseBody,
          status = status,
          errorMessage = "Translation API returned an empty response.",
          latencyMs = System.currentTimeMillis() - start,
        )
        throw TranslationProviderException(
  ...
  private fun emitDebugEvent(
    ...
    listener.onDebugEvent(
      TranslationDebugEvent(
